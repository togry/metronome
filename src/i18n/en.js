...
const en = {
  ...
  scoreUntitled:   (n) => `Score ${n}`,
  tooltipSelectScore: 'Choose which score to work on',
  tooltipAddScore:    'Add a score',
  tooltipDeleteScore: 'Delete this score',
  confirmDeleteScore: 'Delete this score? It cannot be recovered.',
  btnScore:        'SCORE',
  btnThemeDark:    '☀',
  btnThemeLight:   '🌙',
  btnThemeTitleToDaylight: 'Switch to daylight palette',
  btnThemeTitleToNight:    'Switch to night palette',
  btnScoreHelpTitle: 'Score format reference',
  btnHelp:         '?',
  flashLabelMeas:  'MEAS',
  flashLabelBeat:  'BEAT',
  flashLabelUnit:  'UNIT',
...
36||                  # end of score`,
  ...
  helpCredits:     [
    ...
    'Implementation: Claude (Anthropic) — Sonnet 4.5 & 4.6, then Opus 5,',
    '  directed by the above',
  ],
  helpBtnPlay:     '▶',
  ...
  // ── Main help (? in the header) ──────────────────────────────────────────────
  // What is on screen and how to drive it, then just enough of the score format
  // to get started. The score panel has its own ? for the rest.
  helpSections: [
    { h: 'Playback Controls', body: [
      ...
      'BT  Delays the flashes to match Bluetooth audio. Green while taken',
      '  from the device, blue once you move it; \u21BA hands it back.',
    ...
    { h: 'Display', body: [
      'The three dots flash the beat: measure, primary beat, subdivision.',
      'PATTERN shows one dot per click in the current measure, sized by',
      '  weight; hollow dots are rests. When stopped it previews the bar',
      '  the cursor is on, so you can step through with the arrow keys.',
      'The measure readout shows the bar, its meter, and the rehearsal',
      '  mark in force. \u2198 or \u2197 beside the tempo means a curve is running,',
      '  and the figure is the written tempo, not the one playing.',
    ]},
    { h: 'Timeline', body: [
      ...
      'The playhead glides across each bar, and stays dimmed where it',
      '  stopped until you move the cursor.',
    ]},
    { h: 'Loop Region', body: [
    ...
    { h: 'Score Format \u2014 the basics', body: [
      'Each line says what changes at a measure. Only the measure number',
      'and the separator after it are required.',
      '  1| 4/4 1/4=90    # 4/4 at quarter = 90, from m.1',
      '  5| 3/4           # change of meter, tempo unchanged',
      '  9| 7/8 (2+2+3)   # odd meter, grouped 2+2+3',
      ' 12| 1/4=120       # change of tempo, meter unchanged',
      ' 16||              # end of the piece',
      'Leave out the final || and the score loops from m.1 \u2014 the quickest',
      'way to drill a passage.',
      '# or // starts a comment.',
      '',
      'Repeats, D.C./D.S., tempo curves and tuplets have their own',
      'notation: press ? above the score editor for the full description.',
    ], exampleKey: 'practice' },
  ],

  // ── Score format help (? above the score editor) ─────────────────────────────
  scoreHelpTitle: '\u2669 SCORE FORMAT',
  scoreHelpSections: [
    { h: 'Score Format', body: [
      'Each line: measure separator [mark] N/D (grouping) 1/note=BPM',
      'All fields except the measure (number) and separator are optional.',
      'Anything that changes stays in force until changed again.',
      '# or // starts a comment \u2014 can follow content on the same line.',
      'Bracketed measure number \u2192 also becomes rehearsal mark: [12]| 4/4',
      'Text the parser cannot use is shaded in the editor.',
    ]},
    { h: 'Time Signatures', body: [
      'Standard: 4/4  3/4  2/2  6/8  9/8  12/8',
      'Compound (6/8, 9/8, 12/8): primary beat = dotted quarter automatically.',
    ...
    { h: 'DC / DS al Fine / al Coda', body: [
      ...
      'Repeats are not taken on the way back (senza replica).',
    ], exampleKey: 'structure' },
    { h: 'Rit / Accel', body: [
      'Simplest form: rit or accel on the bar where the curve starts,',
      'and the tempo it arrives at on a later bar.',
      '  1| 4/4 1/4=120',
      '  3| rit                 # start slowing here',
      '  7| 1/4=60              # arrival; the rit spreads over m.3-6',
      '',
      'Give the target on the same line when it is not simply the tempo',
      'that follows:',
      '  4| rit 1/4=60          # rit this bar, arriving at 60 bpm',
      '  5| a tempo             # snap back to the tempo before the rit',
      "An explicit target is required when 'a tempo' ends the curve,",
      'since there is then no following tempo mark to take it from.',
      'Tempo is interpolated smoothly on every beat, not once per bar.',
    ], exampleKey: 'rit' },
    { h: 'Tuplets & Rhythmic Patterns', body: [
      ...
      'The cycle counts from the bar where it is written and runs on',
      'through the bars after it; writing a grouping again restarts it.',
    ], exampleKey: 'tuplet' },
  ],
};
